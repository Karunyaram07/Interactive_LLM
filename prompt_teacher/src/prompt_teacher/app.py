...
client = init_hf_client(GATED_MODEL, HF_TOKEN) if HF_TOKEN else init_hf_client(DEFAULT_MODEL)
...
def robustly_improve_prompt(model_name, api_key, prompt_text, metaprompt_name, feedback, chat_history):
    ...
    instruction = (
        f"{mp_obj.explanation}\n\n"
        f"Original Prompt:\n{prompt_text}\n\n"
        f"Your task: Rewrite or expand this according to the PEARL stage objective. "
        f"Provide only the improved or expanded prompt text."
    )
...
def explain_improvement(model_name, api_key, prompt_text, metaprompt_name, improved_prompt, chat_history):
    instruction = (
        f"Explain briefly how this improved prompt supports reflective learning in the PEARL framework.\n\n"
        f"Original Prompt:\n{prompt_text}\n\nImproved Prompt:\n{improved_prompt}\n\n"
        f"Use bullet points."
    )
    try:
...
def generate_learning_outcome(improved_prompt, chat_history):
    instruction = (
        f"Based on this improved prompt:\n{improved_prompt}\n\n"
        "Suggest a short reflective outcome or next learning goal (1–2 sentences)."
    )
    try:
        outcome = client.text_generation(prompt=instruction, max_new_tokens=100, temperature=0.6)
    except Exception as e:
        outcome = f"⚠ Error generating outcome: {e}"
    chat_history.append({"role": "assistant", "content": f"📘 Learning Outcome: {outcome}"})
    return chat_history

# -----------------------------
# Gradio UI
# -----------------------------
with gr.Blocks(title="PromptLearn: AI-based Interactive Teaching using PEARL", theme=gr.themes.Soft()) as app:
    gr.Markdown("## 🎓 **PromptLearn: AI-based Interactive Teaching using PEARL**\n### Learn interactively through the five PEARL stages.")

    with gr.Row():
        with gr.Column(scale=2):
            chat = gr.Chatbot(
                label="PromptLearn Assistant",
                height=520,
                value=[
                    {"role": "user", "content": inital_usr_text},
                    {"role": "assistant", "content": initial_bot_text},
                ],
                show_copy_button=True,
                type="messages"
            ...
            prompt = gr.Textbox(label="Your Prompt", placeholder="Type your question or concept here...")

            with gr.Row():
                explain_btn = gr.Button("Reflect 💭", visible=False)
                replace_btn = gr.Button("Accept Suggestion ✅", visible=False)
                outcome_btn = gr.Button("Next Learning Step 🎯", visible=False)

            improve_btn = gr.Button("✨ Apply PEARL Stage", variant="primary")

        with gr.Column(scale=1):
            pearl_stage = gr.Dropdown(
                label="Select PEARL Stage",
                choices=["Problem Identification", "Exploration", "Application", "Reflection", "Learning Outcomes"],
                value="Problem Identification"
            )
            model_name = gr.Textbox(label="Model", value=DEFAULT_MODEL)
            api_key = gr.Textbox(label="API Token", value="Loaded" if HF_TOKEN else "", interactive=False)
            metaprompt = gr.Radio(
                label="Select Improvement Type",
                choices=[mp.name for mp in metaprompts],
                value=metaprompts[0].name if metaprompts else None
            )
            feedback = gr.Textbox(label="Feedback (optional)", visible=False)

    improved_prompt = gr.Textbox(label="Improved Prompt", visible=False)
    example_expected = gr.Textbox(label="Expected Output", placeholder="Expected answer (for evaluation)")
    example_actual = gr.Textbox(label="Actual Output", placeholder="Paste model output to evaluate")
    eval_btn = gr.Button("Evaluate Example")

    # Interactions
    metaprompt.change(fn=update_widgets, inputs=[metaprompt, feedback], outputs=[improve_btn, feedback])\
        .success(fn=explain_metaprompt, inputs=[chat, metaprompt], outputs=[chat])

    improve_btn.click(
        fn=robustly_improve_prompt,
        inputs=[model_name, api_key, prompt, metaprompt, feedback, chat],
        outputs=[improved_prompt, chat]
    ).success(lambda: [gr.update(visible=True), gr.update(visible=True), gr.update(visible=True)], None, [replace_btn, explain_btn, outcome_btn])

    explain_btn.click(fn=explain_improvement, inputs=[model_name, api_key, prompt, metaprompt, improved_prompt, chat], outputs=[chat])
    replace_btn.click(fn=lambda x: x, inputs=improved_prompt, outputs=prompt)
    outcome_btn.click(fn=generate_learning_outcome, inputs=[improved_prompt, chat], outputs=[chat])
    eval_btn.click(fn=evaluate_example, inputs=[example_expected, example_actual], outputs=[chat])
